...
class IBNConvBlock(nn.Layer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.use_ibn:
            self.ibn = IBN(channels=out_channels, data_format=data_format, name="ibn")
        else:
        ...
        **kwargs
        ...
        for i, channels_per_stage in enumerate(channels):
            ...
            for j, out_channels in enumerate(channels_per_stage):
                strides = 2 if (j == 0) and (i != 0) and (i != len(channels) - 1) else 1
        ...
        self.features.add(
            nn.GlobalAveragePooling2D(data_format=data_format, name="final_pool")
        )

        self.output1 = nn.Dense(units=classes, input_dim=in_channels, name="output1")
...
def get_ibnresnet(
    ...
    **kwargs
    ...
    net = IBNResNet(
        channels=channels, init_block_channels=init_block_channels, **kwargs
    )
    ...
    if pretrained:
        ...
        input_shape = (
            (1,) + (in_channels,) + net.in_size
            if net.data_format == "channels_first"
            else (1,) + net.in_size + (in_channels,)
        )
