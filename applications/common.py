...
def interpolate_im(x, scale_factor=1, out_size=None, data_format="channels_last"):
    """
    ...
    if out_size is None:
        ...
        out_size = (
            tuple(i * scale_factor for i in in_size) if scale_factor != 0 else in_size
        )
...
class PReLU2(nn.PReLU):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(PReLU2, self).__init__(alpha_initializer=alpha_initializer, **kwargs)
    ...
    def build(self, input_shape):
        ...
        channel_axis = (
            1 if is_channels_first(self.data_format) else len(input_shape) - 1
        )
...
class BatchNorm(nn.BatchNormalization):
    ...
    def __init__(
        self, momentum=0.9, epsilon=1e-5, data_format="channels_last", **kwargs
    ):
...
class InstanceNorm(nn.Layer):
    ...
    def _check_size_of_dimensions(self, input_shape):
        ...
        if dim < self.groups:
            raise ValueError(
                "Number of groups ({}) cannot be more than the number of channels ({})".format(
                    self.groups, dim
                )
            )
        if (dim % self.groups) != 0:
            raise ValueError(
                "Number of groups ({}) must be a multiple of the number of channels ({})".format(
                    self.groups, dim
                )
            )
        ...
        **kwargs
        ...
        if self.inst_first:
            self.inst_norm = InstanceNorm(
                scale=True, data_format=data_format, name="inst_norm"
            )
            self.batch_norm = BatchNorm(data_format=data_format, name="batch_norm")
        else:
            self.batch_norm = BatchNorm(data_format=data_format, name="batch_norm")
            self.inst_norm = InstanceNorm(
                scale=True, data_format=data_format, name="inst_norm"
    ...
    def __init__(
        ...
        **kwargs
        ...
        assert (not force_same) or (
            (padding == kernel_size // 2) and (strides == 1) and (dilation == 1)
        )
...
class Conv2d(nn.Layer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.use_pad:
            self.pad = nn.ZeroPadding2D(padding=padding, data_format=data_format)
            # if is_channels_first(data_format):
...
class DenseBlock(nn.Layer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.fc = nn.Dense(
            units=out_channels, use_bias=use_bias, input_dim=in_channels, name="fc"
        )
        if self.use_bn:
        ...
        **kwargs
        ...
        if self.use_crop:
            self.crop = nn.Cropping2D(
                cropping=padding, data_format=data_format, name="crop"
            )
...
class ChannelShuffle2(nn.Layer):
    ...
    def call(self, x):
        return channel_shuffle2(
            x, channels_per_group=self.channels_per_group, data_format=self.data_format
        )
...
class SABlock(nn.Layer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if use_conv:
            ...
            self.fc1 = nn.Dense(units=mid_channels, input_dim=out_channels, name="fc1")
        self.bn = BatchNorm(epsilon=bn_eps, data_format=data_format, name="bn")
        self.activ = nn.ReLU()
        if use_conv:
            self.conv2 = conv1x1(
            ...
            self.fc2 = nn.Dense(units=in_channels, input_dim=mid_channels, name="fc2")
    ...
    def call(self, x, training=None):
        ...
        if is_channels_first(self.data_format):
            ...
            width = x_shape[3]
            x = tf.reshape(
                x, shape=(-1, self.radix, channels // self.radix, height, width)
            )
            ...
            channels = x_shape[3]
            x = tf.reshape(
                x, shape=(-1, height, width, self.radix, channels // self.radix)
            )
        ...
        w = tf.reshape(
            w,
            shape=(-1, self.groups, self.radix, channels // self.groups // self.radix),
        )
...
class PixelShuffle(nn.Layer):
    ...
    def call(self, x, training=None):
        if not tf.executing_eagerly():
            return PixelShuffle2(
                scale_factor=self.scale_factor, data_format=self.data_format
            )(x)
...
class PixelShuffle2(nn.Layer):
    ...
    def call(self, x, training=None):
        ...
        x = tf.nn.depth_to_space(
            input=x, block_size=self.scale_factor, data_format=tf_data_format
        )
        return x
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.pix_shuffle = PixelShuffle(
            scale_factor=scale_factor, data_format=data_format, name="pix_shuffle"
        )
...
class DualPathSequential(SimpleSequential):
    ...
    def __init__(
        ...
        dual_path_scheme_ordinal=(
            lambda block, x1, x2, training: (block(x1, training), x2)
        ),
        **kwargs
    ):
...
class MultiOutputSequential(SimpleSequential):
    ...
    def __init__(
        self, multi_output=True, dual_output=False, return_last=True, **kwargs
    ):
...
class HeatmapMaxDetBlock(nn.Layer):
    ...
    def call(self, x, training=None):
        ...
        indices = tf.cast(
            tf.expand_dims(
                tf.cast(tf.math.argmax(heatmap_vector, axis=vector_dim), np.int32),
                axis=vector_dim,
            ),
            np.float32,
        )
